...
export class ZoeImageParser {
  ...
  getScheduleImageUrl = async (): Promise<string | void> => {
    ...
    try {
      ...
      const elements = Array.from(document.querySelectorAll('p span[style*="color: #ff0000;"]'));

      const targetElement = elements.find((element) => element?.textContent?.includes(targetText));

      const nextParagraph = targetElement?.closest('p')?.nextElementSibling;
      if (nextParagraph) {
    ...
    if (foundImageURL) {
      console.log('Image URL:', foundImageURL);
      return foundImageURL;
    } else {
      console.error('Target text or image not found.');
    }
  };
  ...
  getAllSchedule = async (): Promise<any> => {
    const zoeTodayImageUrl = await this.getScheduleImageUrl();
